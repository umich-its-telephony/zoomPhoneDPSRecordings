	...
	public void configure() throws Exception {
		...
		from("timer:tokenPolling?delay=1000&period={{timer.period}}") // 5 minutes in milliseconds
				...
				.split().jsonpath("$.recordings[*]", true, List.class)
				  ...
				  .setBody(constant(""))
				.to("{{rest.service.url}}")
				.to("file://{{recording.file.dir}}?fileName=$simple{header.callerNumber}_$simple{header.recordingTime}.mp3")
				.log("Downloaded recording of ${header.callerNumber}");

		  from("file://{{recording.file.dir}}?readLock=changed&delete=true")
